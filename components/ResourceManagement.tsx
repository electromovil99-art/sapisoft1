                ...
                <div className="flex-1 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 flex flex-col overflow-hidden">
                    ...
                    <div className="flex-1 overflow-auto p-2">
                        <table className="w-full modern-table">
                            <thead>
                                <tr>
                                    {activeTab === 'marcas' && <><th>Marca</th><th>ID</th><th>Acción</th></>}
                                    {activeTab === 'categorias' && <><th>Categoría</th><th>ID</th><th>Acción</th></>}
                                </tr>
                            </thead>
                            <tbody className="dark:divide-slate-700">
                                {activeTab === 'marcas' && filteredList(brands).map((b: Brand) => (
                                    <tr key={b.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                                        <td className="font-bold text-slate-700 dark:text-slate-200">{b.name}</td>
                                        <td className="font-mono text-slate-400 text-xs">{b.id}</td>
                                        <td className="text-center"><button onClick={() => onDeleteBrand(b.id)} className="text-slate-300 dark:text-slate-600 hover:text-red-500 dark:hover:text-red-400"><Trash2 size={16}/></button></td>
                                    </tr>
                                ))}
                                {activeTab === 'categorias' && filteredList(categories).map((c: Category) => (
                                    <tr key={c.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                                        <td className="font-bold text-slate-700 dark:text-slate-200">{c.name}</td>
                                        <td className="font-mono text-slate-400 text-xs">{c.id}</td>
                                        <td className="text-center"><button onClick={() => onDeleteCategory(c.id)} className="text-slate-300 dark:text-slate-600 hover:text-red-500 dark:hover:text-red-400"><Trash2 size={16}/></button></td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
