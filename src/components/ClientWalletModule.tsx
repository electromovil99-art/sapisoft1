...
const ClientWalletModule: React.FC<ClientWalletProps> = ({ clients, locations, onUpdateClientBalance, onAddClient }) => {
    ...
    const handleOpenTransaction = (client: Client, type: 'Deposit' | 'Withdraw') => { setSelectedClient(client); setTransactionType(type); setAmount(''); setReason(''); setShowModal(true); };
            ...
            {showModal && (
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white dark:bg-slate-800 w-[400px] rounded-2xl shadow-xl p-6 animate-in fade-in zoom-in-95">
                        ...
                        <div className="space-y-4"><div><label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">Monto (S/)</label><input type="number" autoFocus className="w-full p-3 border border-slate-200 dark:border-slate-600 rounded-xl text-2xl font-bold text-slate-800 dark:text-white bg-white dark:bg-slate-700 outline-none focus:border-blue-500" value={amount} onChange={e => setAmount(e.target.value)} placeholder="0.00"/></div><div><label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">Motivo / Referencia</label><input type="text" className="w-full p-3 border border-slate-200 dark:border-slate-600 rounded-xl text-sm bg-white dark:bg-slate-700 text-slate-800 dark:text-white outline-none" value={reason} onChange={e => setReason(e.target.value)} placeholder="Ej. Depósito en efectivo"/></div><button onClick={handleExecuteTransaction} className={`w-full py-3 text-white font-bold rounded-xl mt-2 ${transactionType === 'Deposit' ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-red-600 hover:bg-red-700'}`}>Confirmar Transacción</button></div>
                    </div>
                </div>
            )}
            {showClientModal && (
