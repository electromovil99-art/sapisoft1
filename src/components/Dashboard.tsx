...
import { 
  ShoppingCart, Users, CreditCard, Wrench, CheckCircle, Target, PieChart
} from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ onNavigate, session, cashMovements = [], services = [], clients = [] }) => {
  ...
  const stats = useMemo(() => {
      ...
      // 2. CUENTAS POR COBRAR (DEUDA CLIENTES)
      const totalReceivables = clients.reduce((acc, c) => acc + (c.creditUsed || 0), 0);

      // 3. ESTADISTICAS SERVICIOS (Para el gráfico Donut)
      ...
      <div className="bg-white dark:bg-slate-800 rounded-3xl p-8 shadow-sm border border-slate-200 dark:border-slate-700 relative overflow-hidden flex flex-col md:flex-row items-center md:items-stretch gap-8">
          ...
          <div className="flex-1 z-10 flex flex-col justify-center text-center md:text-left">
              ...
              <p className="text-slate-500 dark:text-slate-400 mb-8 text-lg">
                  Aquí tienes tu avance personal del día.
              </p>

              <div className="flex flex-col sm:flex-row gap-12 justify-center md:justify-start">
                  <div>
                  ...
                  <div>
                      <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Mi Meta Diaria</p>
                      <div className="flex items-baseline gap-3 justify-center md:justify-start">
                          <p className="text-4xl font-bold text-slate-700 dark:text-slate-300">S/ {dailyGoal.toFixed(0)}</p>
                          <span className={`text-sm font-bold px-3 py-1 rounded-full ${goalProgress >= 100 ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300'}`}>
