...
const PrintConfigModule: React.FC = () => {
    ...
    const [configs, setConfigs] = useState<Record<ModuleType, PrintConfig>>({
        VENTAS: { companyName: 'SAPISOFT ERP', ruc: '20601234567', address: 'Av. La Cultura 123, Cusco', phone: '987 654 321', email: 'ventas@sapisoft.com', footerMessage: '¡Gracias por su preferencia! - Conserve este ticket.', logoUrl: '', paperSize: '80mm', autoPrint: true },
        COMPRAS: { companyName: 'SAPISOFT LOGISTICA', ruc: '20601234567', address: 'Av. La Cultura 123, Almacén 2', phone: '', email: 'logistica@sapisoft.com', footerMessage: 'Documento de Control Interno', logoUrl: '', paperSize: 'A4', autoPrint: false },
        SERVICIOS: { companyName: 'SAPISOFT TECNICO', ruc: '20601234567', address: 'Av. La Cultura 123, Taller 5', phone: '987 654 321', email: 'soporte@sapisoft.com', footerMessage: 'Garantía de 30 días por mano de obra. No cubre repuestos.', logoUrl: '', paperSize: '80mm', autoPrint: true }
    });
        ...
        <div className="flex h-full gap-6">
            <div className="w-1/2 flex flex-col gap-6">
                ...
                <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex-1 overflow-y-auto">
                    ...
                    <div className="space-y-4">
                        ...
                        <div><label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1 block">Mensaje Pie de Página</label><textarea className="w-full p-2.5 border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-white rounded-lg text-sm h-20 resize-none outline-none focus:border-indigo-500" value={currentConfig.footerMessage} onChange={e => updateConfig('footerMessage', e.target.value)} /></div>
            ...
            <div className="w-1/2 bg-slate-100 dark:bg-slate-900 rounded-2xl p-8 flex items-start justify-center overflow-y-auto border border-slate-200 dark:border-slate-800 relative">
                ...
                <div className={`bg-white text-slate-800 shadow-2xl p-6 transition-all duration-300 ${currentConfig.paperSize === '80mm' ? 'w-[320px] min-h-[500px]' : 'w-[500px] min-h-[700px]'}`}>
                    ...
                    <div className="my-4 border-t border-dashed border-slate-300"></div>
                    <div className="text-center font-mono text-[10px] text-slate-500"><p>{currentConfig.footerMessage}</p></div>
                    <div className="mt-4 text-center"><div className="inline-block bg-slate-100 p-1"><div className="h-8 w-32 bg-[url('https://upload.wikimedia.org/wikipedia/commons/thumb/d/d0/QR_code_for_mobile_English_Wikipedia.svg/1200px-QR_code_for_mobile_English_Wikipedia.svg.png')] bg-contain bg-no-repeat bg-center opacity-50 mx-auto"></div></div></div>
                </div>
            </div>
        </div>
    );
